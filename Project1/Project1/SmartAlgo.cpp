...
public:
	...
	void setBoard(int player, const char** board, int numRows, int numCols) {
		...
		system("pause");
		for (auto p : normProb)
			PlayersBoard.set(p.first, p.second, 'N');
		for (auto p : conf)
			PlayersBoard.set(p.first, p.second, 'c');
		PlayersBoard.print();
		system("pause");*/
	}
	...
	pair<int, int> attack() {
		...
		if (!isPlayerA)
			SetTextColor(AQUA);
		cout << "Enemy Bluffing? " << enemyFools << endl;
		cout << "HighProb Size:  " << highProb.size() << endl;
		cout << "NormProb Size:  " << normProb.size() << endl;
		cout << "lowProb Size :  " << lowProb.size() << endl;
		cout << "conf Size    :  " << conf.size() << endl;
		cout << "misses 	  :  " << misses << endl;
		cout << "last attack  :  <" << target.first << ", " << target.second << ">       "  << endl;
		//system("pause");
		if (highProb.size()) {
			target = highProb[0];
	...
	void lockOnTarget(int row, int col) {
		...
		if (highProb.size())
			highProb.push_back(make_pair(-1, -1));
